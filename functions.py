...
def deposit(username, amount, fund_name):
    ...
    try:
        ...
        sql_insert = text("""
            INSERT INTO transaction (amount, fund_id, user_id, sent_date) 
            VALUES (:amount, :fund_id, :user_id, NOW())
        """)
...
def withdraw(username, amount, fund_name):
    ...
    user_id = result.fetchone()[0]
    sql_insert = text("""
        INSERT INTO transaction (user_id, amount, fund_id, sent_date) 
        VALUES (:user_id, :amount, :fund_id, NOW())
    """)
...
def create_fund(fund_name, intrest, username):
    ...
    try:
        sql_insert = text("""
            INSERT INTO fund (intrest, fund_name, user_id, create_date) 
            VALUES (:intrest, :fund_name, :user_id, NOW())
        """)
...
def add_to_watchlist(username, fund_name):
    ...
    try:
        sql_insert = text("""
            INSERT INTO watchlist (user_id, fund_id) 
